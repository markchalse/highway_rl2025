...
#         '--seed': 42,  # Set seed
#         '--no-display': True,  # Display settings
#         '--name-from-config': True,  # Use name from config
#         '--recover': False,  # Recover model if applicable
#         '--recover-from': None,  # Option to specify a checkpoint file
#     }
#
#     # 传入硬编码的 environment 和 agent 配置文件路径
#     environment_config = "configs/HighwayEnv/env.json"
#     agent_config = "configs/HighwayEnv/agents/MCTSAgent/baseline.json" #DQNAgent/dqn.json
#
#     # 调用 evaluate 方法
#     for _ in range(int(options['--repeat'])):
#         evaluate(environment_config, agent_config, options)
