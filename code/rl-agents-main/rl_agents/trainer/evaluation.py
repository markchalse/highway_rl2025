...
class Evaluation(object):
    ...
    def __init__(self,
        ...
        self.wrapped_env = RecordVideo(env,
                                       self.run_directory,episode_trigger= lambda e:e % 10 == 0) # e % 10 == 0替换了True，每10个episode保存一个视频。
                                       # (None if self.display_env else lambda e: False))
    ...
    def run_episodes(self):
        for self.episode in range(self.num_episodes):
            ...
            while not terminal:
                ...
                reward, terminal = self.step()    #开始调用step函数
    ...
    def step(self):
        ...
        actions = self.agent.plan(self.observation) #→ 跳转到 MCTSAgent.plan() → AbstractTreeSearchAgent.plan() → 调用 MCTS.plan(...)
        if not actions:
