...
class MergeEnv(AbstractEnv):
    ...
    def _is_terminated(self) -> bool:
        ...
        return self.vehicle.crashed or bool(self.vehicle.position[0] > 370) or self.time > self.config["duration"]
    ...
    def _make_road(self) -> None:
        """
        Make a road vehicle of a straight highway and a merging lane.
    ...
    def _make_vehicles(self) -> None:
        ...
        """
        road = self.road
        rule_based = self.config.get("rule_based", False)
        if rule_based:
            ego_vehicles_type  = utils.class_from_path(self.config["rule_based_type"])
            ego_vehicle = ego_vehicles_type(
                road, road.network.get_lane(("j", "k", 0)).position(110, 0), speed= 25
            )
        else:
