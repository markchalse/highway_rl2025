...
def main():
    ...
    if opts['evaluate']:
        for _ in range(int(opts['--repeat'])):
            evaluate(opts['<environment>'], opts.get('<agent>'), opts)
    elif opts['benchmark']:
        benchmark(opts)
...
def evaluate(environment_config, agent_config, options):
    ...
    use_rule_based = (
        agent_config is None or
        hasattr(base_env, "config") and base_env.config.get("rule_based", False)
    )
...
def benchmark(options):
    ...
    with open(benchmark_filename, 'w') as f:
        json.dump(results, f, sort_keys=True, indent=4)
        logging.info('Benchmark done. Summary written in: {}'.format(benchmark_filename))
