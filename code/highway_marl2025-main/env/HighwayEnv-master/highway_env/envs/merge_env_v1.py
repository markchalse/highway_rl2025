...
class MergeEnv(AbstractEnv):
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool,bool, dict]:
        ...
        obs, reward, terminated, truncated, info = super().step(action) #markchalse
    ...
    def _is_terminated(self) -> bool:
        ...
        #return self.vehicle.crashed or bool(self.vehicle.position[0] > 500)
        #return any(vehicle.crashed for vehicle in self.controlled_vehicles) \
        #       or self.steps >= self.config["duration"] * self.config["policy_frequency"]
        
        #markchalse 0622       
        return any(vehicle.crashed for vehicle in self.controlled_vehicles) or \
            all(vehicle.position[0] > 400 for vehicle in self.controlled_vehicles)
    ...
    def _agent_is_terminal(self, vehicle: Vehicle) -> bool:
        ...
        #       or self.steps >= self.config["duration"] * self.config["policy_frequency"]
        return vehicle.crashed or vehicle.position[0]>600
