...
class RoadObject(ABC):
    ...
    def __init__(self, road, position: Sequence[float], speed: float = 0., heading: float = 0.):
        ...
        """
        self.road = road
        #self.position = np.array(position, dtype=np.float) #numpy 1.23.0
        self.position = np.array(position, dtype=np.float64) #numpy 1.26.4
        self.speed = speed
        self.heading = heading
        # store whether object is hit by any vehicle
        self.hit = False
        
        self.collidable = True #markchalse
        self.diagonal = np.sqrt(self.LENGTH**2 + self.WIDTH**2) #markchalse
        
        self.solid = True #markchalse Collisions have physical effects
